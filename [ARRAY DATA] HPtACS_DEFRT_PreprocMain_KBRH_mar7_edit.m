...
%    else
        ...
        %% Magdalena code
        b = DFRT_EEG.photodiode.data;
        t = DFRT_EEG.photodiode.times;
        %look at the photodiode signal over time
        plot(t, b)
        ...
        %indicating start of stimulus presentation) and photodiode signal going
        %down (trigger indicating disappearence of stimulus)
        lst_on = {}; % Cell array for indices of data points when the photodiode signal goes up
        lst_off = {}; % Cell array for indices of data points when the photodiode signal goes down
        lst_all = {}; % Cell array for indices of data points when the photodiode signal goes up/down
        for i = 1:length(highs)
            if i==1
                continue % skip first point so you have comparison for previous index
            elseif highs(i)==1 && highs(i-1)~=1 && blocks(i) == 1
                lst_on{end+1} = [b(i), i]; % Append new cell containing a tuple
            elseif lows(i)==1 && lows(i-1)~=1 && blocks(i) == 1
                lst_off{end+1} = [b(i), i]; % Append new cell containing a tuple
            end
        end
        ...
        for i = 1:length(highs)
            if i==1
                continue % skip first point so you have comparison for previous index
            elseif highs(i)==1 && highs(i-1)~=1 && blocks(i) == 1
                lst_all{end+1} = [b(i), t(i), 0, i]; % Append new cell containing a list with 0 if turning off
            elseif lows(i)==1 && lows(i-1)~=1 && blocks(i) == 1
                lst_all{end+1} = [b(i), t(i), 1, i]; % Append new cell containing a list with 1 if turning on
            end
        end
        ...
        lst_miss_on = [] %list of indices of photodiode signals (on) that are missing in the EEG.event struct
        lst_miss_off = [] %list of indices of photodiode signals (off) that are missing in the EEG.event struct
        ...
        %list of indices witin the photo-diode signal that are missing in the
        %EEG.event struct
        mss_vals_on = lst_on(lst_miss_on)
        mss_vals_off = lst_off(lst_miss_off)
        
        %attach missing trigger times to the latency list extracted from the EEG.event
        %struct
        new_lats = [lats_ms,mss_vals_on,mss_vals_off]
        
        %sort the new_lats list but keep track of the original index. Then find the
        %new index of the missing triggers
        [sorted_values, sorted_indices] = sort(new_lats);
        idx_mss_all = find(sorted_indices>length(lats_ms))
        
        %generate variable containing the EEG.event struct
        event_new = EEG.event
        
        %introduce empty rows into the event struct at appropriate position.
        %CAUTION: the words written in purple must align with the column titles in
        %your variable event_new. If this is not the case, delete the columns in
        %the variable or add the lables to the below code at the appropriate location.
        ...
        %replace EEG.event by the new variable containing the empty rows
        ...
        % onsets
        index[DFRT_EEG.pdevent,DFRT_EEG.photodiode.data] = parsePhotodiode_sept7_2023(DFRT_EEG.photodiode.data,DFRT_EEG.photodiode.times,true);
        ...
        legend({'Photodiode Channel','Photodiode Events','DIN7 Events'});
        ...
        pdCheckEvts = logical([ISIs<510 0]); % ISIs less than ~500 ms are from the pre-task event check routines
        breakEvts = [ISIs>9000 0];           % ISIs greater than ~9 seconds are taken as inter-block periods
