...
MATLAB_DIR = 'C:/Program Files/MATLAB';
addpath(MATLAB_DIR); %MAP local EEGlab
eeglabFolder = [MATLAB_DIR '/eeglab2023.1/'];
...
% numMissingSub by 2 matrix
% First column is subject ID
% Second column is a list of the missing triggers IN ORDER
PROBLEM_DATA = {...
    'sub034',{'MISSING_Block6_Trial3_Trigger2','MISSING_Block8_Trial3_Trigger3'};...
    'sub027',{'MISSING_Block2_Trial3_Trigger1','EXTRA_Block2_Trial4_Trigger1'};...
    'sub024',{'MISSING_Block2_Trial3_Trigger1','MISSING_Block4_Trial18_Trigger3','Block4_Trial20_Trigger3'}};
...
%    else
        ...
        % This is a plot to show the events that we are keeping after
        % removing these obviously bad events.
        ...
        if ~isempty(problem_subIdx)
            problemTriggers = PROBLEM_DATA{subIdx,2};
            ...
            for problemIdx = 1:numProblems
                ...
                blockIdx = str2double(blockIdx_str(7:end));
                ...
                if strcmpi(addOrRm_BOOL,'EXTRA')
                    ...
                    % If we are missing the first trigger, then count
                    % backwards from the second trigger.
                    missing_firstTrigger = -4;
                    % if we are missing the second trigger, then count
                    % forwards from the first trigger.
                    missing_secondTrigger = 4;
                    % If we are missing the third trigger, then count
                    % forwards from the second trigger.
                    missing_thirdTrigger = 5.5;

                    numTotalEvents = length(pdevent);
                    new_pdevent = pdevent(1:problem_eventIdx-1);

                    if triggerIdx == 1
                        nextEvent_time = pdevent(problem_eventIdx).latency;
                        new_eventTime = nextEvent_time + missing_firstTrigger;
                    elseif triggerIdx == 2
                    ...
                    new_pdevent(problem_eventIdx) = struct('latency',new_eventTime);
                    new_pdevent((problem_eventIdx+1):(numTotalEvents+1)) = pdevent(problem_eventIdx:end);
                end
            end
        end
