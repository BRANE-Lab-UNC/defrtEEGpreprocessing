...
dbstop in HPtACS_DEFRT_PreprocMain_KBRH_mar14_edit.m at 260 %breakpoint on executable code
...
print([SUB_PROC_EEG '/' rootFilename '_revisedeventParse.png'],'-dpng')
        ...
        indexesToKeep = ~ismember(1:length(DFRT_EEG.pdevent), indexesToRemove);
        DFRT_EEG.pdevent = DFRT_EEG.pdevent(indexesToKeep);
        ...
        for i = 1:length(indicesToAdd)
            ...
            if mod(idx, 3) == 1
                newLatency = DFRT_EEG.pdevent(idx + 1).latency - 4000;
            elseif mod(idx, 3) == 2
                newLatency = DFRT_EEG.pdevent(idx + 1).latency - 5000;
            elseif mod(idx, 3) == 0
                newLatency = DFRT_EEG.pdevent(idx - 1).latency + 5000;
            end
            ...
            DFRT_EEG.pdevent = [DFRT_EEG.pdevent(1:idx-1), newEvent, DFRT_EEG.pdevent(idx:end)];
