...
dbclear('HPtACS_DEFRT_PreprocMain_RH_jun02_2024.m');
%dbstop in HPtACS_DEFRT_PreprocMain_RH_apr24_2024.m at 390 %breakpoint on beginning of executable code
dbstop in HPtACS_DEFRT_PreprocMain_RH_jun02_2024.m at 475 %breakpoint on ending of executable code
...
% Use this variable if you'd prefer to name subjects manually. This may be
% useful for the case when you are testing one or two subjects individually
SUBJECTS = {'63'};
% Use this variable if you'd prefer subjects to be named automatically. This
% should be used as a more robust method of running all code at once.
excludeSUBJECTS = {'69', '75'};
subjectFileInfo = dir(fullfile(rawEEG_dir, 'DEFRT_*_EEG'));
if isempty(SUBJECTS)
    for i = 1:length(subjectFileInfo)
        SUBJECT = subjectFileInfo(i).name;
        % Regular expression to extract the numbers from the filenames
        subjectStr = regexp(SUBJECT, 'DEFRT_(\d+)_EEG', 'tokens');
        if ~isempty(subjectStr)
            subjectNumber = subjectStr{1}{1};
            % Check if the subject number is not in the excluded list
            if ~ismember(subjectNumber, excludeSUBJECTS)
                % Add the subject number to SUBJECTS
                SUBJECTS{end+1} = subjectNumber;
            end
        end
    end
end
...
%    else
        ...
        triggersAdded = 0;
        triggersRemoved = 0;
        % Extract the first 18 practice triggers
        pdevent = DFRT_EEG.pdevent;
        practiceTriggers = pdevent(1:18);
        
        % Work with the rest of the events after  18 triggers in practice
        pdevent = pdevent(19:end);
        ...
        for block = 1:6
            ...
            if ~isempty(files)
                    ...
                    % Now you can access the file through fullPath
                    ...
                    for trial = 1:20
                        ...
                        for trigger = 1:3
                            ...
                            % Trials are made up of 3 triggers, and each
                            % Trigger is made up of 1 trigger
                            % There will be events added that should be
                            % reflected in the count of triggers and then
        ...
        indicesToRemove = 1:numel(pdevent) > 360;
        pdevent(indicesToRemove) = [];
        ...
        yData = ones(size(eventLatencies));
        
        % plotting
        figure;  % Open a new figure window
        plot(eventLatencies, yData, 'o', 'MarkerEdgeColor', 'b', 'MarkerFaceColor', 'r');  % Blue outline, red fill
        ylim([0.8 1.2]);  % set Y limits to keep markers at '1' centered
        xlim([min(eventLatencies) max(eventLatencies)]);  % adjust depending on the range of your data
        xlabel('Event Time (latency)');
        ylabel('Event Indicator');
        title('Event Plot');
        grid on;  % turn on the grid for better visibility
        ax = gca;
        ax.YTick = [];  % remove y-axis ticks since all events are at '1'

        % now again with the new pdevent
        eventLatencies = [pdevent.latency];  % extract latencies from the struct array
        ...
        if ~isempty(problem_subIdx)
            ...
            for problemIdx = 1:numProblems
                ...
                if strcmpi(addOrRm_BOOL,'EXTRA')
                    ...
                    base = ['Task_data/DEEFRT/DEFRT_sub0' subject '/DEFRT_sub0' subject '_' blockIdxStr];
                    files = dir([ROOT base '_*.mat']);
                    if ~isempty(files)
                        fullPath = fullfile(files(1).folder, files(1).name);
                    ...
                    % Check if problem_eventIdx is 0
                    if numTriggersInPractice + 1  % First actual trigger
                        % If so, set previousEvent_time to 0
                        previousEvent_time = 0;
                    else
                        % Otherwise, retrieve the time of the previous event normally
                        previousEvent_time = pdevent(problem_eventIdx-1).latency;
                    end
